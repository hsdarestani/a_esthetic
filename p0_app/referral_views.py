...
from platform_app.models import AuditLog, Campaign, GiftCard, Referral, WalletAccount, WalletTransaction
...
def _send_referral_email(referral, request):
    ...
    body = json.dumps({"invited_email": referral.invited_email, "referral_code": referral.code}).encode("utf-8")
    remote = Request(
        REFERRAL_RELAY_URL,
        data=body,
        method="POST",
        headers={"Authorization": auth, "Content-Type": "application/json", "Accept": "application/json", "User-Agent": "A-Esthetic-Points-Referral/2.0"},
    )
    try:
...
def mobile_club(request):
    ...
    if request.method == "POST":
        ...
        referral = Referral.objects.filter(referrer=user, invited_email__iexact=invited_email).order_by("-created_at").first()
        if referral and referral.status == "rewarded":
            return JsonResponse({"ok": False, "error": "already_referred", "referral_id": referral.pk}, status=409)
        if not referral:
            referral = Referral.objects.create(referrer=user, code=_new_code(), invited_email=invited_email, reward_coins=REFERRAL_POINTS)
    ...
    payload = {
        ...
        "points": wallet.coin_balance,
        "referral_points": REFERRAL_POINTS,
        "campaigns": [{"id": c.pk, "name": c.name, "message": c.message, "ends_at": c.ends_at.isoformat()} for c in campaigns],
        "giftcards": [{"id": c.pk, "code": c.code, "status": c.status} for c in giftcards],
        "referrals": [{"id": r.pk, "code": r.code, "email": r.invited_email, "status": r.status, "reward_points": r.reward_coins} for r in referrals],
    }
    if email_sent is not None:
        payload.update({"email_sent": email_sent, "referral_id": referral_id, "points_awarded": points_awarded})
